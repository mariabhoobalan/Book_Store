...
public class QueryUtil {
    ...
    public static List<Books> extractFeatureFromJson(String booksJSON) {
        ...
        List<Books> z_books = new ArrayList<>();
        ...
        try {
            ...
            for (int i = 0; i < booksArray.length(); i++) {
                ...
                String xtitle = volumeinfo.getString("title");
                String xurl = volumeinfo.getString("previewLink");
                String xauthor = "Author   :";

                //check if authors is available; else default the result
                if (volumeinfo.has("authors")) {
                    JSONArray authorArray = volumeinfo.getJSONArray("authors");
                    for (int x = 0; x < authorArray.length(); x++) {
                        xauthor = xauthor + authorArray.getString(x) + "  ";
                    }
                } else {
                    xauthor = xauthor + "Author list not available";
                }
                //write the books detail in the list
                Books book = new Books(xtitle, xauthor, xurl);
                z_books.add(book);
            }
